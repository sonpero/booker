#!/usr/bin/env python3
# Author: Antonios Tsolis (Alamot) for get_epub_cover
...
def get_epub_cover(epub_path):
    ...
    with zipfile.ZipFile(epub_path) as z:
        ...
        try:
            ...
            '''
            cover_href = \
            t.xpath("//opf:manifest/opf:item[@id='" + cover_id + "']",
        ...
        if not cover_href:
            ...
            try:
                cover_href = \
                t.xpath("//opf:manifest/opf:item[@properties='cover-image']",
                        namespaces=namespaces)[0].get("href")
            except IndexError:
                pass
        ...
        if not cover_href:
            ...
            try:
                ...
                # and get the attribute "href":
                cover_page_href = \
                t.xpath("//opf:manifest/opf:item[@id='" + cover_page_id + "']",
                        namespaces=namespaces)[0].get("href")
                # In order to get the full path for the cover page,
                # we have to join rootfile_path and cover_page_href:
                cover_page_path = os.path.join(os.path.dirname(rootfile_path),
                                               cover_page_href)
                ...
                cover_href = t.xpath("//xhtml:img", namespaces=namespaces)[
                    0].get("src")
            except IndexError:
                pass
